 ...
 * 
 * 포트(Port)
 * -IP 주소는 네트워크 상에서 컴퓨터를 찾을 때만 사용하고 컴퓨터 내에서 실행하는 프로그램을 선택하기 위해 포트를 사용한다.
 * - 클라이언트는 서버 연결 요청 시 IP 주소와 포트 번호를 알고 있어야 한다.
 * - 포트는 0~ 65535r까지의 범위를 갖는다.
 * 
...
public class InetAddressTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("getHostAdderess() : " + ip.getHostAddress()); // 호스트에 ip 주소를 반환
			
			System.out.println();
			
			ip = InetAddress.getLocalHost();
			System.out.println("getHostName() : " + ip.getHostName()); //DESKTOP-Q4DT0NS
			System.out.println("getHostAddress() : " + ip.getHostAddress()); //192.168.10.19
