...
public class B_Casting {
	...
	/*
	 *  자동 형 변환(묵시정 형 변환)
	 *  - 자동으로 형 변환이 이루어지기 때문에 따로 형변환 하지 않아도 된다.
	...
	public void autoCasting() {
		...
		float f =1; // 4byte(표현 가능한 수의 범위가 더 크기 때문에 자동 형변환이 가능)
		double d = 1; // 8byte
	 ...
	 * [표현법] (자료형) 데이터;
	 * 
	 * - 범위가 큰 크기의 자료형의 데어터를 작은 크기의 자료형으로 변환하려고 할 때 사용한다.
	 * - 강제 형변환의 경우 데이터 손실이 발생한다.
	 */
	
	public void casting() {
		double d = 4.12345678904567890; // 15자리수만 표현 가능
		float f = (float) d; // 6자리수만 표현 가능
