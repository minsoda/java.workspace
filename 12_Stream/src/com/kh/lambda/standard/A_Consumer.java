...
import java.util.function.Consumer; //ctrl + shift + o
import java.util.function.DoubleConsumer;
import java.util.function.ObjIntConsumer;

/*
 * 표준 함수적 인터페이스
 * - 자바 8부터 번번히 사용되는 함수적 인터페이스는 java.util.function 표준 API 패키지로 제공
 * - 자바 8부터 추가되거나 변경된 API는 이 표준 함수적 인터페이스들을 매개 타입으로 사용
 * - 용도에 따라 크게 Consumer, Supplier, Function, Operator, Predicate로 구분
 * 
 * Consumer
 * - Consumer 함수적 인터페이스는 리턴 값이 없는 accept() 추상 메서드를 자기고 있다. 
...
//		a.method3();
	...
	public void method3() {
		...
		DoubleConsumer dConsumer = value -> System.out.println("DoubldConsumer는 하나의 double 타입의 값을 받아서 소비한다 : " + value);
		dConsumer.accept(3.14);
	}
	
	/*
	 * ObjIntConsumer<T> : T 객체와 int 값을 받아 소비
	 * ObjLongConsmer<T> :T 객체와 long 값을 받아 소비
	 * ObjDoublieCunsumer<T> :T 객체와 double값을 받아 소비
	 * */
	public void method4() {
